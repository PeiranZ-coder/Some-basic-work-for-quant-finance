...
$$
A = \left[ a_1 \quad a_2 \quad \cdots \quad a_n \right]
$$
...
> 📌 金融含义：$A$ 表示 $n$ 支证券在 $m$ 个时间点的收益矩阵，每一列是一支证券，每一行是一个时间点。
...
> 📌 金融含义：$x$ 是投资组合权重向量，$Ax$ 是组合在各时间点的收益。
...
> 📌 金融含义：协方差矩阵通过转置计算，$\Sigma = \frac{1}{T} X X^T$。
...
> 📌 金融含义：若证券收益线性相关，则存在冗余证券，市场不完全，无法对所有风险对冲。
...
> 📌 金融含义：$\text{rank}(A) < n$ 意味着市场不完全，存在无法被复制的收益结构。

---

## 1.9 Nonsingular Matrices & Inverse 非奇异矩阵与逆

**定义：** 方阵 $A$ 可逆，若存在 $A^{-1}$ 使得：
...
> 📌 金融含义：Arrow-Debreu 证券对应单位矩阵的列，每种证券只在一种状态下支付。
...
| 概念 | 核心公式 | 金融应用 |
|---|---|---|
| 列向量 | $v \in \mathbb{R}^n$ | 证券收益序列 |
| 矩阵 | $A \in \mathbb{R}^{m \times n}$ | 多资产收益矩阵 |
| 转置 | $(A^T)_{ij} = A_{ji}$ | 协方差矩阵计算 |
| 协方差矩阵 | $\Sigma = \frac{1}{T}XX^T$ | 风险度量 |
| 线性无关 | $c_1v_1+\cdots+c_kv_k=0$ | 无冗余证券 |
| 秩 | $\text{rank}(A)$ | 市场完全性判断 |
| 可逆矩阵 | $AA^{-1}=I$ | 完全市场复制 |
| Arrow-Debreu | 单位矩阵的列 | 状态价格定价 |
